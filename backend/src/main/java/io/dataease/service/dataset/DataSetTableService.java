...
public class DataSetTableService {
    ...
    public Map<String, Object> getPreviewData(DataSetTableRequest dataSetTableRequest,
                                              Integer page, Integer pageSize,
                                              List<DatasetTableField> extFields,
                                              DatasetRowPermissionsTreeObj extTree) throws Exception {
        ...
        // 处理request jinajava变量
        List<SqlVarParamDTO> sqlVarList = handleJinJavaDataSetTableVariables(dataSetTableRequest);
        ...
        if (StringUtils.equalsIgnoreCase(datasetTable.getType(), DatasetType.DB.name()) || StringUtils.equalsIgnoreCase(datasetTable.getType(), DatasetType.API.name())) {
            if (datasetTable.getMode() == 0) {
                ...
                String sql = jinJavaUtils.handleJinJavaTemplate(jinJavaUtils.encodeToString(datasourceRequest.getQuery()),
                                                                sqlVarList);
                map.put("sql", sql);
                ...
                QueryProvider qp = ProviderFactory.getQueryProvider(ds.getType());
                datasourceRequest.setQuery(qp.createQueryTableWithPage(table, fields, page, pageSize,
                                                                       realSize, false, ds, null,
                                                                       rowPermissionsTree));
                String sql = jinJavaUtils.handleJinJavaTemplate(jinJavaUtils.encodeToString(datasourceRequest.getQuery()),
                                                                sqlVarList);
                map.put("sql", sql);
            ...
            if (datasetTable.getMode() == 0) {
                ...
                map.put("sql", jinJavaUtils.encodeToString(datasourceRequest.getQuery()));
                datasourceRequest.setPage(page);
                datasourceRequest.setFetchSize(Integer.parseInt(dataSetTableRequest.getRow()));
                datasourceRequest.setPageSize(pageSize);
                datasourceRequest.setRealSize(realSize);
                ...
                String table = TableUtils.tableName(dataSetTableRequest.getId());
                QueryProvider qp = ProviderFactory.getQueryProvider(ds.getType());
                datasourceRequest.setQuery(qp.createQueryTableWithPage(table, fields, page,
                                                                       pageSize, realSize,
                                                                       false, ds, null,
                                                                       rowPermissionsTree));
                String sql = jinJavaUtils.handleJinJavaTemplate(jinJavaUtils.encodeToString(datasourceRequest.getQuery()),
                ...
                try {
                    datasourceRequest.setQuery(qp.createQueryTableWithLimit(table, fields,
                                                                            Integer.valueOf(dataSetTableRequest.getRow()),
                                                                            false, ds, null, rowPermissionsTree));
                    dataSetPreviewPage.setTotal(jdbcProvider.getData(datasourceRequest).size());
                } catch (Exception e) {
                    logger.error(e.getMessage());
                    DEException.throwException(Translator.get("i18n_ds_error") + "->" + e.getMessage());
            ...
            QueryProvider qp = ProviderFactory.getQueryProvider(ds.getType());
            datasourceRequest.setQuery(qp.createQueryTableWithPage(table, fields, page,
                                                                   pageSize, realSize, false,
                                                                   ds, null, rowPermissionsTree));
            map.put("sql", jinJavaUtils.encodeToString(datasourceRequest.getQuery()));
            try {
                data.addAll(jdbcProvider.getData(datasourceRequest));
            } catch (Exception e) {
            ...
            if (datasetTable.getMode() == 0) {
                ...
                QueryProvider qp = ProviderFactory.getQueryProvider(ds.getType());
                datasourceRequest.setQuery(qp.createQueryTableWithPage(sql, fields, page,
                                                                       pageSize, realSize,
                                                                       false, ds, null,
                                                                       rowPermissionsTree));
                sql = jinJavaUtils.handleJinJavaTemplate(jinJavaUtils.encodeToString(datasourceRequest.getQuery()),
                                                         sqlVarList);
                map.put("sql", sql);
                datasourceRequest.setPage(page);
                datasourceRequest.setFetchSize(Integer.parseInt(dataSetTableRequest.getRow()));
    ...
    private List<SqlVariableDetails> getSqlVariableDetails(String type, List<DatasetTable> datasetTables) {
        ...
        switch (type) {
            case "DATE":
                sqlVariableDetails = sqlVariableDetails.stream()
                                                       .filter(item -> item.getType().get(0)
                                                                           .contains("DATETIME"))
                                                       .collect(Collectors.toList());
            ...
            case "NUM":
                sqlVariableDetails = sqlVariableDetails.stream()
                                                       .filter(item -> item.getType().get(0).contains("LONG")
                                                                                || item.getType().get(0).contains("DOUBLE"))
                                                       .collect(Collectors.toList());
                sqlVariableDetails.forEach(item -> item.setAlias(item.getVariableName()));
                break;
        }
    ...
    private void handleSelectItem(SelectItem selectItem, String dsType) throws Exception {
        if (!(selectItem instanceof SelectExpressionItem)) {
            return;
        }
        SelectExpressionItem selectExpressionItem = (SelectExpressionItem) selectItem;
        Expression expr = selectExpressionItem.getExpression();
        if (expr == null || !(expr instanceof SubSelect)) {
            return;
        }
        SelectBody selectBody = ((SubSelect) expr).getSelectBody();
        ...
        selectExpressionItem.setExpression(subSelect);
    }
    ...
    private List<SqlVarParamDTO> handleJinJavaDataSetTableVariables(DataSetTableRequest dataSetTableRequest) {
    ...
    public ExcelFileData excelSaveAndParse(MultipartFile file, String tableId, Integer editType) throws Exception {
        ...
        if (StringUtils.isNotEmpty(tableId)) {
            ...
            if (editType == 1) {
                ...
                List<String> extFieldsRefNames = datasetTableFields.stream().filter(datasetTableField -> extFieldsRefIds.contains(datasetTableField.getId())).map(DatasetTableField::getOriginName)
                                                                   .collect(Collectors.toList());
                for (ExcelSheetData excelSheetData : excelSheetDataList) {
                    List<TableField> tableFields = excelSheetData.getFields();
                    List<String> newFields = tableFields.stream().map(TableField::getRemarks).collect(Collectors.toList());
    ...
    private ExpressionDeParser getExpressionDeParser(StringBuilder stringBuilder) {
        ExpressionDeParser expressionDeParser = new ExpressionDeParser(null, stringBuilder) {
            ...
            public void visit(InExpression inExpression) {
                ...
                if (inExpression.getRightExpression() != null && hasVariable(inExpression.getRightExpression().toString())) {
                    stringBuilder.append(SubstitutedSql);
                    return;
                }
